...
// 7_x86_64(5, 8) returns 8 and align_to_x86_64(11, 8) returns 16.
...
static void push_struct(Type *ty) {
  int sz = align_to_x86_64(ty->size, 8);
  ...
  for (Node *arg = node->args; arg; arg = arg->next) {
    ...
    case TY_UNION:
      if (ty->size > 16) {
        ...
        stack += align_to_x86_64(ty->size, 8) / 8;
      } else {
...
static void assign_lvar_offsets(Obj *prog) {
  for (Obj *fn = prog; fn; fn = fn->next) {
    ...
    for (Obj *var = fn->locals; var; var = var->next) {
      ...
      bottom += var->ty->size;
      bottom = align_to_x86_64(bottom, align);
      var->offset = -bottom;
    }

    fn->stack_size = align_to_x86_64(bottom, 16);
  }
}
