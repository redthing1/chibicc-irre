...
// align_to_riscv64(5, 8) returns 8 and align_to_riscv64(11, 8) returns 16.
...
static void push_struct(Type *ty) {
  int sz = align_to_riscv64(ty->size, 8);
  ...
  for (Node *arg = args; arg; arg = arg->next) {
    ...
    case TY_UNION:
      if (ty->size > 16) {
        ...
        stack += align_to_riscv64(ty->size, 8) / 8;
      } else {
...
static void assign_lvar_offsets(Obj *prog) {
  for (Obj *fn = prog; fn; fn = fn->next) {
    ...
    for (Obj *var = fn->locals; var; var = var->next) {
      ...
      bottom = align_to_riscv64(bottom, var->align);
      var->offset = -bottom;
      bottom += var->ty->size;
    }

    fn->stack_size = align_to_riscv64(bottom, 16);
  }
}
